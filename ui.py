    ...
    st.markdown(
        ...
        <nav class="ada-nav" aria-label="Primary navigation">
          ...
          <div class="nav-actions">
            <a class="nav-link" href="https://port-folio-main-window.vercel.app/" target="_blank">Portfolio ↗</a>
...
def render_dashboard(dataframe: pd.DataFrame, roles: ColumnRoles) -> None:
    ...
    with movement_columns[1]:
        ...
        if not heat.empty and len(heat.columns) >= 2:
            ...
            st.markdown(
                '<div class="empty-state">Select a segment and date to unlock the intensity heatmap.</div>',
                unsafe_allow_html=True,
            )

    st.markdown('<div class="section-label" style="margin-top:1.5rem">Custom Chart Studio (Bar, Line, Pie, Scatter, Box)</div>', unsafe_allow_html=True)
    render_chart_studio(dataframe, roles)


def render_chart_studio(dataframe: pd.DataFrame, roles: ColumnRoles) -> None:
    """Allow custom generation of Bar, Line, Pie, Scatter, and Boxplot charts."""
    cols = list(dataframe.columns)
    if not cols:
        return

    num_cols = [c for c in cols if pd.api.types.is_numeric_dtype(dataframe[c])]
    cat_cols = [c for c in cols if not pd.api.types.is_numeric_dtype(dataframe[c])]

    studio_cols = st.columns(4)
    chart_type = studio_cols[0].selectbox("Chart Type", ["Bar", "Line", "Pie", "Scatter", "Box"])
    x_col = studio_cols[1].selectbox("X Axis / Category", cols, index=cols.index(roles.dimension) if roles.dimension in cols else 0)
    y_col = studio_cols[2].selectbox("Y Axis / Metric", num_cols if num_cols else cols, index=0)
    color_col = studio_cols[3].selectbox("Group / Color (Optional)", ["None", *cat_cols], index=0)

    color_arg = color_col if color_col != "None" else None

    try:
        if chart_type == "Bar":
            fig = px.bar(dataframe.head(500), x=x_col, y=y_col, color=color_arg, title=f"{y_col} by {x_col}")
        elif chart_type == "Line":
            fig = px.line(dataframe.head(500), x=x_col, y=y_col, color=color_arg, title=f"{y_col} trend by {x_col}")
        elif chart_type == "Pie":
            fig = px.pie(dataframe.head(500), names=x_col, values=y_col, title=f"{y_col} share by {x_col}")
        elif chart_type == "Scatter":
            fig = px.scatter(dataframe.head(500), x=x_col, y=y_col, color=color_arg, title=f"{y_col} vs {x_col}")
        else:  # Box
            fig = px.box(dataframe.head(500), x=x_col, y=y_col, color=color_arg, title=f"{y_col} distribution by {x_col}")

        st.plotly_chart(style_chart(fig, height=360), width="stretch", config={"displayModeBar": False})
    except Exception as error:
        st.warning(f"Could not render {chart_type} chart with selected columns: {error}")
...
def render_chat_answer(result: QueryAnswer) -> None:
    """Render one answered question with its table, chart, calculation, and generated SQL."""
    ...
    if result.sql:
        with st.expander("🔍 AI Reasoning & Generated SQL"):
            st.markdown("**Deterministic Math Plan:**")
            st.code(result.calculation, language="text")
            st.markdown("**Equivalent DuckDB SQL:**")
...
def render_footer() -> None:
    st.markdown(
        """
        <footer class="footer"><span>ADA · Automated Data Analyst · Built by Kartikeya Mishra</span>
        <span><a href="https://port-folio-main-window.vercel.app/" target="_blank">Portfolio ↗</a></span></footer>
        """,
        unsafe_allow_html=True,
    )

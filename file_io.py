...
def read_multiple_files(files_data: list[tuple[str, bytes]]) -> dict[str, pd.DataFrame]:
    """Parse multiple uploaded files and return a dict of {table_name: DataFrame}."""
    result = {}
    for filename, contents in files_data:
        try:
            df = read_tabular_file(contents, filename)
            clean_key = Path(filename).stem.lower()
            clean_key = "".join(c if c.isalnum() else "_" for c in clean_key).strip("_") or "data"
            result[clean_key] = df
        except Exception:
            continue
    return result
